...
3. Состояние базы данных
   ...
   Количество активных соединений: Если соединений к базе данных становится слишком много, это может привести к её перегрузке. Я узнал об этом, когда наша база данных просто перестала отвечать на запросы. Теперь я всегда слежу за этим показателем.

   Время выполнения запросов: Медленные запросы — это как пробки на дороге. Они тормозят всё вокруг. Если запросы к базе данных выполняются слишком долго, это повод оптимизировать их.
